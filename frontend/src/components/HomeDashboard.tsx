    ...
    async function load() {
      try {
        ...
        const json = await fetchJsonWithTimeout<OverviewResponse>(
          ...
          { cache: "no-store", timeoutMs: 75_000 }
        );
        setData(json);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    async function loadOutliers() {
      try {
        ...
        const json = await fetchJsonWithTimeout<OutliersResponse>(
          `/api/outliers?${params.toString()}`,
          { cache: "no-store", timeoutMs: 75_000 }
        );
  ...
  useEffect(() => {
    ...
    async function loadVideos() {
      try {
        ...
        const json = await fetchJsonWithTimeout<VideosResponse>(
