...
export async function GET(request: NextRequest) {
  try {
    ...
    // Deduplicate by latest scrape per video_id.
    ...
    for (const row of rows) {
      ...
      if (!existing || scrape > String(existing[0] ?? "")) {
    ...
    console.error("Error in /api/outliers:", err);
    return NextResponse.json(
      { error: "Failed to load outliers" },
      { status: 500 }
    );
  }
}
