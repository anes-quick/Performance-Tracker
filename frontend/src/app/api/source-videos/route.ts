...
export async function GET(request: NextRequest) {
  try {
    ...
    const sourceId = String(searchParams.get("sourceId") ?? "").toUpperCase().trim();
    ...
    for (const row of latestByVideo.values()) {
      ...
      const rowSourceId = String(row[9] ?? "").toUpperCase().trim() || "UNKNOWN";
      const rowSourceName = String(row[10] ?? "").trim();
      ...
      items.push({
        ...
        sourceName: rowSourceName || rowSourceId || "Unknown source",
      });
    }

    items.sort((a, b) => b.views - a.views);
    return NextResponse.json({ sourceId, period, count: items.length, items });
  } catch (err) {
    console.error("Error in /api/source-videos:", err);
    return NextResponse.json(
      { error: "Failed to load source videos" },
      { status: 500 }
    );
  }
}
